...
% J=(-1/m)*sum(sum((y.*log(a3)+(1-y).*log(1-a3))));
% 
% regularTheta1=Theta1(:,2:end);
% regularTheta2=Theta2(:,2:end);
% 
% addition= (lambda/(2*m))*(sum(sum(regularTheta1.^2))+sum(sum(regularTheta2.^2)));
% 
% J=J+addition;
% 
% del1 = zeros(size(Theta1));
...
%     
%     sacma3=a3i-yi;
%     sacma2=Theta2'*sacma3'.*sigmoidGradient([1;Theta1 * a1i']);
%     
%     del1 = del1 + sacma2(2:end)*a1i;
% 	del2 = del2 + sacma3' * a2i;
%     
% end
% 
% 
% Theta1_grad = 1/m * del1 + (lambda/m)*[zeros(size(Theta1, 1), 1) regularTheta1];
% Theta2_grad = 1/m * del2 + (lambda/m)*[zeros(size(Theta2, 1), 1) regularTheta2];
% 
% 
% 
% 
